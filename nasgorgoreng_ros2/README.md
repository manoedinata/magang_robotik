...
3.  Run all nodes:
    ...
    3.d. In terminal 4, run the UDP publisher to ESP32 node:

    ```bash
    source install/setup.bash
    ros2 run lane_detector publish_udp_esp32_node --ros-args -p esp32_ip:=<ESP32_IP_ADDRESS>

    # You need to replace <ESP32_IP_ADDRESS> with the actual IP address of your ESP32 device.
    ```
